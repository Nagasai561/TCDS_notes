...
Let $cal(A) in RR^(m times n times p)$ be a third-order tensor.
...
$
op("unfold") : RR^(m times n times p) arrow RR^((m p) times n)
$

- *Folding* is the inverse operation
$
op("fold") : RR^((m p) times n) arrow RR^(m times n times p)
$
]

=== Example: Unfolding and folding
...
quad in RR^((m p) times n)
$

Applying folding recovers the tensor:
$
op("fold")(op("unfold")(cal(A))) = cal(A)
$

#line()

== T-product of Tensors
...
The *circulant matrix* generated by $arrow(v)$ is
$
op("circ")(arrow(v)) =
mat(
  v_1, v_3, v_2;
...
$
op("bcirc")(cal(A)) = op("circ")(op("unfold")(cal(A)))
$
]
...
Let
$
cal(A) in RR^(m times n times p),
quad
cal(B) in RR^(n times r times p)
$
...
The *t-product* is defined as
$
cal(A) star cal(B)
=
op("fold")(
  op("bcirc")(cal(A)) dot op("unfold")(cal(B))
)
$
]
...
$
cal(A) star cal(B) in RR^(m times r times p)
$
...
Let
$
cal(A) = [A_1, A_2, A_3] in RR^(5 times 4 times 3),
quad
cal(B) = [B_1, B_2, B_3] in RR^(4 times 2 times 3)
$
...
which produces a tensor in $RR^(5 times 2 times 3)$.